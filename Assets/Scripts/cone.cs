...
public class cone : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (hauteur != 0 && hauteurTronc <= hauteur)
            // Centre face haute
            vertices[meridiens + 1] = new Vector3(0, hauteur - (hauteur - hauteurTronc), 0);
        else
            vertices[meridiens + 1] = new Vector3(0, 0, 0);
        ...
        for (int i = 1; i <= meridiens; i++)
        {
            ...
            if (hauteur != 0 && hauteurTronc <= hauteur)
                // Haute
                vertices[i + meridiens + 1] = new Vector3(rayon * (1 - hauteurTronc / hauteur) * Mathf.Sin((2 * Mathf.PI * i) / meridiens), hauteur - (hauteur - hauteurTronc), rayon * (1 - hauteurTronc / hauteur) * Mathf.Cos((2 * Mathf.PI * i) / meridiens));
            else
                vertices[i + meridiens + 1] = new Vector3(rayon * Mathf.Sin((2 * Mathf.PI * i) / meridiens), 0, rayon * Mathf.Cos((2 * Mathf.PI * i) / meridiens));
